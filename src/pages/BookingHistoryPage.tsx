...
type TabType = 'All' | 'Approved' | 'Rejected' | 'Cancelled';
...
export default function BookingHistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      setIsLoading(true);
      try {
        // Get all reservations then exclude Pending on client side
  ...
  const canceledCount = history.filter((h) => h.status === 'Cancelled' || h.status === 'Rejected').length;
    ...
    <DashboardLayout role="admin" userName={adminName}>
      <div className="p-8 max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-500 text-[15px]">Review past reservation history from API backend.</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
            <div className="w-10 h-10 rounded-lg bg-sky-50 flex items-center justify-center mb-6 border border-sky-100"><Calendar size={20} className="text-[#0065A1]" /></div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Total (Non-Pending)</p>
            <h3 className="text-3xl font-bold text-slate-900">{totalBookings}</h3>
          </div>
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
            <div className="w-10 h-10 rounded-lg bg-emerald-50 flex items-center justify-center mb-6 border border-emerald-100"><CheckCircle2 size={20} className="text-emerald-600" /></div>
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
            <div className="w-10 h-10 rounded-lg bg-rose-50 flex items-center justify-center mb-6 border border-rose-100"><XCircle size={20} className="text-rose-600" /></div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Rejected / Cancelled</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6 border-b border-slate-100 flex flex-col lg:flex-row justify-between items-center gap-4">
            <div className="flex bg-slate-50 p-1 rounded-lg border border-slate-200">
              {(['All', 'Approved', 'Rejected', 'Cancelled'] as TabType[]).map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${activeTab === tab ? 'bg-white text-[#0065A1] shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
                  {tab}
                </button>
              ))}
            </div>
            <div className="flex items-center gap-3 w-full lg:w-auto">
              ...
              <button className="flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-600 text-sm font-semibold rounded-lg hover:bg-slate-50 whitespace-nowrap">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={5} className="py-8 text-center"><Loader2 className="animate-spin mx-auto mb-2 text-[#0065A1]" />Loading...</td></tr>
                  ...
                  <tr><td colSpan={5} className="py-12 text-center text-slate-500">No matching history found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
