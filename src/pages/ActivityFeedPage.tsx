...
import { useAuth } from '../contexts/AuthContext'; // FIX: Tambahkan ini
import { 
  CalendarPlus, XCircle, RotateCw, ArrowLeftSquare, 
  Clock, Users, ArrowRight, Check, CheckCheck
} from 'lucide-react';
...
interface ActivityItem {
  id: string; type: ActivityType; iconType: ActivityType;
  title: string; time: string; actor: string; role?: string;
  actionText: string; isRead: boolean; timeRange?: string;
  peopleCount?: string; badgeLabel?: string; badgeStyle?: string;
  rescheduleFrom?: string; rescheduleTo?: string;
}

const initialActivities: ActivityItem[] = [
  {
    id: '1', type: 'approvals', iconType: 'approvals', title: 'New Booking: Room 302',
    time: '2 mins ago', actor: 'Sarah Jenkins', role: 'Senior Analyst',
    actionText: "reserved the Executive Suite for a 'Quarterly Strategy Session'.",
    timeRange: '14:00 - 16:30', peopleCount: '12 People', isRead: false,
  },
  {
    id: '2', type: 'canceled', iconType: 'canceled', title: 'Canceled: Studio B',
    time: '18 mins ago', actor: 'Mark Thompson', role: 'Creative Director',
    actionText: "released the reservation due to schedule conflict.",
    badgeLabel: 'HIGH PRIORITY RELEASE', badgeStyle: 'bg-red-50 text-red-700 border-red-100', isRead: false,
  },
  {
    id: '3', type: 'rescheduled', iconType: 'rescheduled', title: 'Rescheduled: Tech Hub',
    time: '1 hour ago', actor: 'Development Team A',
    actionText: "moved 'Sprint Planning' from Tuesday 10:00 to Wednesday 09:00.",
    rescheduleFrom: 'Aug 14, 10:00', rescheduleTo: 'Aug 15, 09:00', isRead: false,
  },
  {
    id: '4', type: 'returned', iconType: 'returned', title: 'Returned Early: Executive Suite 302',
    time: '3 hours ago', actor: 'Sarah Jenkins',
    actionText: "ended their booking 30 minutes ahead of schedule. Room is now available.",
    badgeLabel: 'RETURNED EARLY', badgeStyle: 'bg-indigo-50 text-indigo-700 border-indigo-100', isRead: false,
  }
];

export default function ActivityFeedPage() {
  // FIX: Ambil data dari Supabase agar konsisten
      ...
      <div className="p-8 max-w-7xl mx-auto">
        ...
        <div className="flex gap-8 border-b border-slate-200 mb-8 px-2">
          {(['all', 'approvals', 'canceled', 'returned'] as ActivityTab[]).map((tab) => {
            const labels = { all: 'All Activity', approvals: 'Approvals', canceled: 'Canceled', returned: 'Returned Early' };
            return (
              <button
                key={tab}
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            {filteredActivities.length > 0 ? (
              filteredActivities.map((act) => {
                ...
                return (
                  <div key={act.id} className={`rounded-xl p-6 shadow-sm border flex gap-5 transition-all relative ${act.isRead ? 'bg-white border-slate-100' : 'bg-[#F4FAFF] border-blue-200'}`}>
                    {!act.isRead && (<div className="absolute top-6 right-6 w-2.5 h-2.5 bg-[#0088FF] rounded-full shadow-[0_0_8px_rgba(0,136,255,0.4)]"></div>)}
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 border ${bg}`}>{icon}</div>
                    <div className="flex-1 pr-6">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className={`text-[17px] font-semibold ${act.isRead ? 'text-slate-800' : 'text-slate-900'}`}>{act.title}</h3>
                        <span className={`text-sm ${act.isRead ? 'text-slate-400' : 'text-[#0088FF] font-medium'}`}>{act.time}</span>
                      </div>
                      ...
                      {act.badgeLabel && (<span className={`inline-block px-2.5 py-1 text-[11px] font-bold rounded uppercase tracking-wide border ${act.badgeStyle}`}>{act.badgeLabel}</span>)}
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-[#0065A1] rounded-xl p-6 shadow-md text-white">
              ...
              <div className="space-y-5">
                <div>
                  <div className="flex justify-between items-center mb-2"><span className="text-sky-100 text-sm">Bookings</span><span className="font-bold text-[17px]">86</span></div>
                  <div className="w-full h-1.5 bg-[#004A77] rounded-full overflow-hidden"><div className="w-[70%] h-full bg-white rounded-full"></div></div>
                </div>
                <div>
                  <div className="flex justify-between items-center mb-2"><span className="text-sky-100 text-sm">Cancellations</span><span className="font-bold text-[17px]">12</span></div>
                  <div className="w-full h-1.5 bg-[#004A77] rounded-full overflow-hidden"><div className="w-[15%] h-full bg-white rounded-full"></div></div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-sky-50 border-l-4 border-[#0088FF] p-4 rounded-r-lg">
                  ...
                  <button className="text-[11px] font-bold text-[#0088FF] hover:text-blue-800 tracking-wider uppercase">Review Change</button>
                </div>
                <div className="bg-[#FFF8F1] border-l-4 border-orange-400 p-4 rounded-r-lg">
                  ...
                  <button className="text-[11px] font-bold text-orange-700 hover:text-orange-800 tracking-wider uppercase">View Queue</button>
                </div>
              </div>
            </div>
          </div>
